...
class Solution:
    def mergeTwoLists(self, list1: Optional[ListNode], list2: Optional[ListNode]) -> Optional[ListNode]:
        # Iterative Solution
        # Time - O(n) = O(m) + O(n) where m is list1 and n is list2 looping through both
        # Memory - O(1) - Since we are not creating new nodes, using existing nodes
        # Only created 2 nodes  
